...
class Move {
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.jumpTail() == null) {
            if (move0._row1 == move1._row0
                    && move0._col1 == move1._col0) {
                return move(move0._col0, move0._row0,
                        move0._col1, move0._row1, move1);
            } else {
                return move(move0._col0, move0._row0,
                        move0._col1, move0._row1,
                        move(move0._col1, move0._row1,
                                move1._col0, move1._row0, move1));
            }
        } else {
            ArrayList<Move> moveList = new ArrayList<>();
            Move counter = move0._nextJump;
            Move prev = move(move0._col0, move0._row0,
                    move0._col1, move0._row1);
            Move combined = move1;
            while (counter != null) {
                prev = move(prev._col1, prev._row1,
                        counter._col0, counter._row0);
                moveList.add(move(counter._col0, counter._row0,
                        counter._col1, counter._row1));
                counter = counter._nextJump;
            }
            for (int i = moveList.size() - 1; i >= 0; i--) {
                combined = move(moveList.get(i), combined);
            }
            return move(move0._col0, move0._row0,
                    move0._col1, move0._row1, combined);
     ...
     */
    boolean isLeftMove() {
        int x = (index(_col1, _row1)) - (index(_col0, _row0));
        return !isJump() && x == -1 && _row0 == _row1
                && validSquare(_col1, _row0);
    }
     ...
     */
    boolean isRightMove() {
        int z = index(_col1, _row1);
        int y = index(_col0, _row0);
        int x = (index(_col1, _row1)) - (index(_col0, _row0));
        return !isJump() && x == 1 && _row0 == _row1
                && validSquare(_col1, _row0);
    }
    ...
    private void toString(Formatter out) {
        if (jumpTail() == null) {
            out.format("" + _col0 + "" + _row0 + "-" + _col1 + "" + _row1);
        } else {
            out.format("" + _col0 + "" + _row0 + "-" + jumpTail().toString());
        }
    }
